...
class DadataOSINTTool:
    ...
    ACTIONS = {
        # Free suggestion/reference methods.
        "address": ("suggest", "address", False),
        "address_by_id": ("findById", "address", False),
        "reverse_geocode": ("geolocate", "address", False),
        "ip_location": ("iplocate", "address", False),
        "postal_by_index": ("findById", "postal_unit", False),
        "postal_nearby": ("geolocate", "postal_unit", False),
        "party": ("suggest", "party", False),
        "party_by_id": ("findById", "party", False),
        "bank": ("suggest", "bank", False),
        "bank_by_id": ("findById", "bank", False),
        "fio": ("suggest", "fio", False),
        "email": ("suggest", "email", False),
        "passport_issuer": ("suggest", "fms_unit", False),
        # These endpoints can consume a paid balance and are opt-in.
        "clean_address": ("clean", "address", True),
        "clean_fio": ("clean", "name", True),
        "clean_phone": ("clean", "phone", True),
        "clean_email": ("clean", "email", True),
        "clean_passport": ("clean", "passport", True),
        "clean_vehicle": ("clean", "vehicle", True),
        "brand_by_inn": ("brand", "brand", True),
    }
    ...
    def __init__(
        ...
        self.api_key = (
            api_key or os.getenv("DADATA_API_KEY") or dotenv.get("DADATA_API_KEY", "")
        ).strip()
        self.secret_key = (
            secret_key
            or os.getenv("DADATA_SECRET_KEY")
            or dotenv.get("DADATA_SECRET_KEY", "")
        ).strip()
        allow_paid = (
            os.getenv("DADATA_ALLOW_PAID")
            or dotenv.get("DADATA_ALLOW_PAID", "false")
        )
    ...
    def _read_dotenv(cls) -> dict[str, str]:
        values: dict[str, str] = {}
        try:
            lines = cls.APP_DIR.joinpath(".env").read_text(
                encoding="utf-8-sig"
            ).splitlines()
        except OSError:
            return values
    ...
    def _headers(self, require_secret: bool = False) -> dict[str, str]:
        headers = {
            ...
            "User-Agent": "DEEPX-DaData-OSINT/1.0",
        }
        if require_secret:
    ...
    def _payload(
        ...
        if action in {"reverse_geocode", "postal_nearby"}:
            payload = {
                "lat": self._number(lat, "lat", -90, 90),
                "lon": self._number(lon, "lon", -180, 180),
                "count": self._count(count),
            }
            if "radius_meters" in options:
                payload["radius_meters"] = int(
                    self._number(options["radius_meters"], "radius_meters", 1, 1000)
                )
        ...
        if action == "ip_location":
            return {"ip": text, **{
                key: options[key]
                for key in ("language", "division")
                if options.get(key)
            }}

        payload["count"] = self._count(count)
        allowed_options = {
            "kpp", "branch_type", "type", "status", "language", "division",
            "from_bound", "to_bound", "locations", "locations_boost", "filters",
        }
    ...
    async def _logo_by_domain(self, query: Any) -> str:
        ...
        if not re.fullmatch(
            r"(?:[a-z0-9а-яё](?:[a-z0-9а-яё-]{0,61}[a-z0-9а-яё])?\.)+"
            r"[a-zа-яё]{2,63}",
            domain,
            flags=re.IGNORECASE,
        ):
        ...
        try:
            content = await asyncio.to_thread(
                self._request_sync,
                url,
                {"Accept": "image/*", "User-Agent": "DEEPX-DaData-OSINT/1.0"},
                None,
                "GET",
            )
            ...
            return json.dumps({
                "ok": True,
                "source": "DaData company logos",
                "action": "logo_by_domain",
                "domain": domain,
    ...
    async def run(
        ...
        if not self.api_key:
            return self._json_error(
                "DADATA_API_KEY is not configured in .env.", action
            )
        if action == "logo_by_domain":
            return await self._logo_by_domain(query)
        if action not in self.ACTIONS:
            return self._json_error(
                "Unsupported action. Available actions: "
                + ", ".join(sorted((*self.ACTIONS, "logo_by_domain"))),
                action,
            )
        ...
        if paid and not self.allow_paid:
            return self._json_error(
                "This action may consume a paid DaData balance and is disabled. "
                "Set DADATA_ALLOW_PAID=true only after the user explicitly approves paid calls.",
                action,
            )
        if paid and not self.secret_key:
            return self._json_error(
                "DADATA_SECRET_KEY is required for this action.", action
            )
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self._request_sync,
                url,
                self._headers(require_secret=paid),
                payload,
            )
            return json.dumps({
                "ok": True,
