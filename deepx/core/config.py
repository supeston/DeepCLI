...
TOOL_DESCRIPTIONS = {
    ...
    "run_cmd": '- run_cmd: {"command": "команда", "inputs": "опциональный stdin с \\n"}. Используй только команды cmd.exe, не PowerShell. Не запускай через run_cmd GUI-приложения, серверы и другие долгоживущие процессы: для них используй run_background_cmd, затем task_status/task_log.',
